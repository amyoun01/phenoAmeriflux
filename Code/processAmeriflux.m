function [sitedataH]=processAmeriflux(fn)
...
csvimport(char(fn), 'columns', {'TIMESTAMP','TA_F','LE_F_MDS','H_F_MDS','GPP_DT_VUT_REF', 'SW_IN_F', 'NETRAD'});
...
sitedataH=data(:, [1 2 5 7 6]); %cleaning up data so now it is [ time temp nee gpp r qFlag]
sitedataH=[sitedataH EF month day yr]; %adding time components
Tann=nanmean(sitedataH(:, 2)); %annual mean temperature

idx=sitedataH(:, 4)<-1;%-9999 means missing data in fluxnet/ameri-flux, so the following is just some filtering to remove missing/bad data
